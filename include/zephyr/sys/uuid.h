 ...
 * @param data_size The size of the out buffer. Should be at least UUID_SIZE.
 * @param out The UUID where the result will be written.
 *
 * @retval 0 if the UUID has been correctly parsed and stored in @p out
 * @retval -EINVAL if @p data is null or @p data_size is too small
 */
int uuid_from_buffer(const uint8_t *data, size_t data_size, uuid_t out);
 ...
 * @param out_size The size of the out buffer. Should be at least UUID_STR_LEN chars.
 *
 ...
 * @param uuid The UUID to convert to string.
 * @param out A pointer to a previously allocated buffer where the result will be written.
 * @param out_size The size of the out buffer. Should be at least UUID_BASE64URL_LEN chars.
 *
 * @retval 0 if the UUID has been converted and written in @p out
 * @retval -EINVAL if @p out is null or @p out_size is too small
 */
